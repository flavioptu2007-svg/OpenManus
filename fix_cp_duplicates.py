...
while content.count(cp_css_marker) > 0:
    ...
    if nxt_css > 0:
        end_candidates.append(nxt_css)
    nxt_style = content.find("</style>", start)
    if nxt_style > 0:
        end_candidates.append(nxt_style)
    if end_candidates:
...
for i, pos in enumerate(positions[:-1]):
    ...
    section_end = min(
        (
            content.find(js_marker, pos + 1)
            if content.find(js_marker, pos + 1) > 0
            else len(content)
        ),
        (
            content.find("</script>", pos) + len("</script>")
            if content.find("</script>", pos) > 0
            else len(content)
        ),
    )
