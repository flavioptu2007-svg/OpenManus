...
class QuestaoSchema(Schema):
    ...
    materia = fields.Str(validate=validate.Length(max=60))
    serie = fields.Str(validate=validate.Length(max=30))
    ...
    def validate_texto(self, value):
        if not value.strip():
            raise ValidationError(
                "O texto da questão não pode ser vazio ou apenas espaços."
            )
...
class ProvaCreateSchema(Schema):
    ...
    question_ids = fields.List(fields.Int(), load_default=None, allow_none=True)
...
class PaginationSchema(Schema):
    ...
    class Meta:
        unknown = EXCLUDE  # tolera filtros extras (limit, offset, materia, serie...)
