...
def find_marked_answers(image_path: str,
    ...
    """
    answers, _ = find_marked_answers_with_confidence(image_path, grid_definition, threshold)
    return answers
    ...
    try:
        ...
        for q_num, alts in grid_definition['questoes'].items():
            ...
            for alt, coords in alts.items():
                roi = binary[coords['top']:coords['bottom'],
            ...
            if best_density >= threshold and best_alt:
                ...
                valid_marks = [a for a, d in densities.items()
                               if d >= threshold * 0.7]
...
def calcular_nota(respostas: dict, gabarito: dict,
                   escala: float = 10.0) -> tuple[float, int]:
    """
