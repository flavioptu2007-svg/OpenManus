    ...
    for folha in folhas:
        ...
        writer.writerow(
            [
                ...
                folha.aluno_info or "",
                (
                    folha.data_processamento.strftime("%Y-%m-%d %H:%M")
                    if folha.data_processamento
                    else ""
                ),
                folha.nota or 0,
                folha.acertos or 0,
        ...
        for f in folhas:
            table_data.append(
                [
                    ...
                    str(f.acertos or 0),
                    (
                        f.data_processamento.strftime("%d/%m/%Y")
                        if f.data_processamento
                        else ""
                    ),
                ]
            )
...
def export_to_csv(provas: List[Prova]) -> str:
    ...
    writer = csv.writer(output)
    writer.writerow(
        ["id", "nome", "data", "qr_code_info", "marked_answers", "status", "created_at"]
    )
