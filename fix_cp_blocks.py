...
for i, (s, e) in enumerate(blocks):
    ...
    print(
        f"  Block {i+1} ({e-s} chars): {', '.join(cp_items) if cp_items else 'no CP code'}"
    )
    ...
    for ri in remove_blocks:
        ...
        if cp_start >= 0:
            ...
            for m in [
                ...
                if m_pos > 0:
                    if (
                        ...
                        or (cp_start + 10 + m_pos) < next_section_start + blocks[ri][0]
                    ):
            ...
            if next_section_start > 0:
                ...
                print(
                    f"  Removing CP code from Block {ri+1}: offset {cp_start}-{next_section_start} (file: {file_start}-{file_end})"
                )
